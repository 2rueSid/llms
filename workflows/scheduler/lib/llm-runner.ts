...
export abstract class LLMRunner {
	abstract streamResponse(input: UserInput[]): Promise<response>;
}

export class CodexRunner extends LLMRunner {
	readonly codex = new Codex();

	override async streamResponse(input: UserInput[]): Promise<response> {
